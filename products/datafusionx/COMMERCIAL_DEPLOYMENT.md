...
- `scripts/build-commercial-release.sh` 可生成两种交付形态：默认本地离线包会包含镜像 tar；CI 公开商业包会推送固定版本 GHCR 镜像，并只打包 Docker Compose、Helm Chart、`CUSTOMER_DELIVERY_CHECKLIST.md`、`checksums.txt`、`release-manifest.json` 和 `release-manifest.sig`。
- 商业后端镜像构建阶段会生成客户/交付批次水印和核心文件完整性 manifest，并用公司 Ed25519 发布私钥签名；Web、Celery Worker 和 Celery Beat 启动时会使用 `COMMERCIAL_INTEGRITY_PUBLIC_KEY` 校验 manifest 签名和关键文件 hash。商业构建标记会强制启用完整性校验，不能通过把 `COMMERCIAL_INTEGRITY_REQUIRED=false` 作为普通环境变量来关闭。
...
export DATAFUSIONX_COMMERCIAL_CUSTOMER_ID=<客户编码，公开快照可使用 public-release>
export DATAFUSIONX_COMMERCIAL_DELIVERY_ID=<交付批次，默认使用版本号>
export DATAFUSIONX_COMMERCIAL_DISTRIBUTION_MODE=<public-snapshot 或 customer-private>
...
正式发布时还应在交付单中记录镜像 digest、客户编码和交付批次。签名私钥不得进入本仓库、镜像和客户部署包。

客户专属交付必须满足以下规则：

- `DATAFUSIONX_COMMERCIAL_CUSTOMER_ID` 必须使用真实客户编码，不能使用 `public-release`。
- `DATAFUSIONX_COMMERCIAL_DISTRIBUTION_MODE` 应为 `customer-private`，未显式设置时脚本会根据客户编码自动推断。
- 镜像应推送到私有 registry、私有 GHCR package，或以离线镜像包交付；不得把客户专属镜像和客户专属部署包同步到 Public-Releases。
- 客户包中的 `CUSTOMER_DELIVERY_CHECKLIST.md` 必须随交付单一起留档。
...
- 手动 `workflow_dispatch`：填写 `version` 时生成指定正式版本；留空时生成快照版本；可填写 `customer_id` 和 `image_repository` 生成客户专属交付。
- 当 `customer_id` 不是 `public-release` 时，必须关闭 `publish_public_repo`，且 `image_repository` 必须指向客户专属私有镜像仓库前缀；workflow 会在发布前校验这些约束。
...
5. 校验部署包不包含源码、私钥、浮动镜像标签或源码构建配置，并校验客户交付清单、商业构建水印和后端受保护目录源码清理状态。
...
公开仓库不应作为客户专属镜像的唯一保护边界；正式客户交付必须使用私有 GHCR package、客户 token、限期下载链接或离线镜像包，并记录压缩包 sha256、release manifest 签名、后端镜像 digest、前端镜像 digest、客户编码、交付批次和 License 授权号。
...
- DataFusionX 客户包内置 `tools/commercial-manifest.py`、`release-manifest.json` 和 `release-manifest.sig`，并使用固定商业发布验签公钥校验发布包；SagittaDB 主要使用商业镜像 manifest 和 zip sha256 校验。
- DataFusionX 当前客户包包含 Docker Compose、Helm Chart、环境模板和商业部署说明；SagittaDB 还包含独立 `LEGAL-NOTICE.md`、`upgrade.sh`、`verify-license.sh`、Nginx 配置和截图目录。后续如 DataFusionX 面向客户交付需要一键升级或独立法务说明，可按 SagittaDB 的客户包资产补齐。
...
- 客户专属交付通过镜像 label 和商业构建水印记录客户编码与交付批次，泄露镜像可追溯到交付来源。
- workflow 会阻止客户专属交付误发到 Public-Releases，且要求客户专属交付使用私有镜像仓库前缀。
